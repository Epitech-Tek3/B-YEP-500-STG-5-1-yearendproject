 ...
 */
async function makeRequest(url: string, access_token: string, client_id: string) {
  const config = {
    method: "get",
    url: url,
  ...
  if (res.data.data[0].type == "live") channel.send(res.data.data[0].title);
}

module.exports = function () {
  makeRequest(
    "https://api.twitch.tv/helix/users?id=125387632",
    "z2695l7wr5k339v3bhsovjtwmyw9bx",
    "ehdum2rn1klq2ttofjqpuea5e6ypn6"
  );
};
